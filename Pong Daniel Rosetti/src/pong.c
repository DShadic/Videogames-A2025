...
void render_pong(struct Pong pong, struct Fonts fonts)
{
    ...
    if (pong.state == START)
    {
        al_draw_text(fonts.large_font, al_map_rgb(255, 255, 255), TABLE_WIDTH / 2, TABLE_HEIGHT / 2, ALLEGRO_ALIGN_CENTER, "Press 1 for Singleplayer");
        al_draw_text(fonts.large_font, al_map_rgb(255, 255, 255), TABLE_WIDTH / 2, TABLE_HEIGHT / 2 + 50, ALLEGRO_ALIGN_CENTER, "Press 2 for Multiplayer");
    }
    else if (pong.state == SERVE)
    {
